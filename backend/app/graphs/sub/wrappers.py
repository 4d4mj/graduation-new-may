...
from app.graphs.sub.patient_analysis import build_patient_analysis_graph
import logging
...
patient_analysis_graph = build_patient_analysis_graph().compile(checkpointer=None)
...
async def run_patient_analysis_subgraph(state, config):
    """Run the patient analysis subgraph on the given state with logging."""
    logger.info("↪ patient_analysis subgraph, incoming final_output=%r", state.get("final_output"))
    new_state = await patient_analysis_graph.ainvoke(state, config=config)
    logger.info("↩ patient_analysis subgraph, outgoing final_output=%r", new_state.get("final_output"))
    return new_state
