...
async def run_patient_supervisor_subgraph(state, config):
    ...
    if MEMORY_DEBUG and thread_id:
        logger.info(f"[MEMORY] Patient supervisor subgraph using thread_id: {thread_id}")

    # Log the incoming state for debugging
    logger.info("↪ patient_supervisor subgraph, incoming final_output=%r",
    ...
    logger.info("↩ patient_supervisor subgraph, outgoing final_output=%r intent=%r scheduling=%r",
