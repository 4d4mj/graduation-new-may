...
async def get_patient_allergies_info(
    ...
    async with tool_db_session() as db:
        try:
            ...
            allergies = await get_allergies_for_patient(db, patient_user_id=patient.user_id)
            
            if not allergies:
                return f"No known allergies recorded for {patient.first_name} {patient.last_name}"
            
            response_lines = [f"Recorded allergies for patient {patient.first_name} {patient.last_name}"]
