...
class ScheduleGoogleCalendarInput(BaseModel):
    attendee_email: str = Field(..., description="Email address of the person to invite to the Google Calendar event.")
    summary: str = Field(..., description="The title or summary of the Google Calendar event (e.g., 'Meeting with Dr. Smith').")
    event_time_str: str = Field(description="The time for the appointment for TOMORROW, in HH:MM format (e.g., '14:30' for 2:30 PM).")
    duration_hours: float = Field(default=1.0, description="Duration of the appointment in hours (e.g., 0.5 for 30 minutes, 1 for 1 hour).")
    description: Optional[str] = Field(None, description="Optional detailed description or message for the event body.")
    # timezone_str is optional here; if not provided by LLM, we'll try to use injected user_tz or a default
    timezone_str: Optional[str] = Field(None, description="Optional: IANA timezone name for the event (e.g., 'America/New_York', 'Asia/Beirut'). If omitted, the user's configured timezone or a system default will be used.")
    send_notifications: bool = Field(default=True, description="Whether to send email notifications to attendees.")
    calendar_id: str = Field(default='primary', description="The calendar ID to add the event to. Defaults to 'primary'.")
...
def _get_calendar_service_sync():
    ...
    if not TOKEN_FILE_PATH.exists():
        ...
        return None, f"Configuration error: Google Calendar token file not found at {TOKEN_FILE_PATH}."

    try:
        creds = Credentials.from_authorized_user_file(str(TOKEN_FILE_PATH), SCOPES)
    except Exception as e:
        logger.error(f"Google Calendar: Error loading credentials from {TOKEN_FILE_PATH}: {e}")
        return None, f"Error loading Google Calendar credentials: {e}. Ensure the token file is valid."

    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            logger.warning(f"Google Calendar: Credentials in {TOKEN_FILE_PATH} are expired. Attempting refresh (might fail without client secrets).")
            ...
            logger.error(f"Google Calendar: Could not load valid credentials from {TOKEN_FILE_PATH}.")
            return None, "Invalid or missing Google Calendar credentials. Token may be expired or corrupted."

    try:
        service = build('calendar', 'v3', credentials=creds)
        logger.info("Google Calendar service object created successfully using token.json.")
        return service, None
    except HttpError as error:
        logger.error(f'Google Calendar: An API error occurred building the service: {error}')
        if error.resp.status == 401:
            return None, "Google Calendar authentication error (401): The token is likely invalid or expired, and refresh failed or was not possible."
        return None, f"API error building Google Calendar service: {error.resp.status} - {error.content}"
    except Exception as e:
        logger.error(f'Google Calendar: An unexpected error occurred during service build: {e}')
...
async def schedule_google_calendar_event(
    ...
    timezone_str: Annotated[Optional[str], InjectedState("user_tz")] = None, # Injects user_tz from agent state
    ...
    if final_timezone_str is None: # Check if it was actually provided or resolved by InjectedState
        ...
        logger.info(f"No specific timezone provided or injected, using default: {final_timezone_str}")
    else:
    ...
    def _sync_book_appointment():
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Invalid event_time_str format: '{event_time_str}'. Using 09:00.")
            ...
            end_datetime_local = start_datetime_local + datetime.timedelta(hours=duration_hours)

            start_time_rfc3339 = start_datetime_local.isoformat()
            end_time_rfc3339 = end_datetime_local.isoformat()

            logger.info(f"Attempting to book Google Calendar event for: {start_datetime_local.strftime('%Y-%m-%d %H:%M:%S %Z%z')}")

            event_body = {
                'summary': summary,
                'description': description if description else summary,
                'start': {'dateTime': start_time_rfc3339, 'timeZone': final_timezone_str},
            ...
            logger.error(f'Google Calendar API error: {api_error}. Details: {api_error.content}')
            if api_error.resp.status == 401:
                return ("Google Calendar authentication error: The token is likely invalid or expired. "
            ...
            logger.error(f"An unexpected error occurred during Google Calendar booking: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    print("To run the test, uncomment `asyncio.run(main_test())` and configure `invite_email` and `token.json`.")
