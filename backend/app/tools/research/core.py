...
class MedicalRAG:
    def __init__(self) -> None:
        ...
        if not self.vector_store or not self.llm:
            raise RuntimeError("RAG initialisation failed – vector store or LLM missing")

        logger.info("MedicalRAG ready%s",
    ...
    async def process_query(
        ...
        k = agent_settings.rag.reranker_top_k

        # 1. similarity search
        docs_scores = await search_vector_store(query, k=k * 3)
        if not docs_scores:
            return {"response": AIMessage(content="I found no relevant information."),
